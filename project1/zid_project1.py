...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    suffix = '_prc.dat'
    pth = os.path.join(DATDIR, f'{tic}{suffix}')
    result = []
    with open(pth) as f:
        for line in f:
            result.append(line)
...
def line_to_dict(line):
    ...
    prefix=0
    for col in COLUMNS:
        # print(prefix, prefix + COLWIDTHS[col])
        result[col]=line[prefix:prefix+COLWIDTHS[col]]
        prefix+=COLWIDTHS[col]
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        if not tickers_lst:
            raise Exception('The ticker list is an empty list.')
        for tic in tickers_lst:
            if tic_exchange_dic.get(tic) is None:
                raise Exception(f'{tic} is not a ket of tic_exchange_dic.')
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    inBlock={}
    for tic in tickers_lst:
        inBlock['exchange']=tic_exchange_dic[tic]
        excinfo=read_dat(tic)
        dailyLst=[]
        for line in excinfo:
            dailyLst.append(line_to_dict(line))
        inBlock['data']=dailyLst
        result[tic]=inBlock
